...
exports.importQuestions = async (req, res) => {
  try {
    ...
    if (req.body.preview === 'true' || req.body.preview === true) return res.json({ success: true, count: documents.length, questions: documents });
    const saved = await Question.insertMany(documents, { ordered: true });
    res.status(201).json({ success: true, count: saved.length, message: `${saved.length} questions imported.`, messageHindi: `${saved.length} प्रश्न आयात किए गए।` });
